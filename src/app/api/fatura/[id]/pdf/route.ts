...
export async function GET(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const plano = await prisma.plano.findFirst({ where: { id: contrato?.planoId } });
    ...
    page.drawText(`Mensalidade: ${(valorFormatado || 7990/100).toLocaleString("pt-BR",{style:"currency",currency:"BRL"})}`, { x: margin + padding, y: planoY + planoHeight - 45, size: 10, font });
    ...
    return new Response(pdfBytes, {
      status: 200,
      headers: { "Content-Type": "application/pdf", "Content-Disposition": `attachment; filename=fatura-${vencimento}.pdf` }
    });
